...
const DashboardReportsMissedCalls = () => {
...
const handleRowClick = (report) => {
 ...
 const fetchMissedCall = async (startDate, endDate) => {
  try {
    const response = await fetch(`${config.SOS_elivator}/callLists`, {
      ...
      // If you want to filter by status directly in request
      body: JSON.stringify({elevatorId,startDate,endDate }),
    });

    const result = await response.json();

    // Assuming the response structure is { callLists: [...] }
    const data = result.callLists || [];

    console.log(data, "All Call Data");

    // Filter only missed calls based on call_status (optional if backend doesn’t filter)
    const MissedCall = data.filter(
      (calls) => calls.call_status?.trim().toLowerCase() === 'missed'
    );

    console.log(MissedCall, "Filtered Missed Calls");

    setReports(MissedCall);

    if (MissedCall.length > 0) {
      setSelectedLiftId(MissedCall[0].id);
      setSelectedReport(MissedCall[0]);
    }
  } catch (error) {
...
const handleDateChange = (startDate, endDate) => {
  if (startDate && endDate) {
    fetchMissedCall(startDate, endDate);
  } else {
    fetchMissedCall();
  }
};
...
const handleSearchClick = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${config.SOS_elivator}/getMonthlyMissed`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      <SimpleGrid columns={1} spacing={4} textAlign="center" style={{marginRight:'20px'}}>
        <Box bg="white" boxShadow="md" borderRadius="lg" p={6} mb={0} w="100%">
          <Text fontSize={{ base: '18px', md: '20px' }} mb={8} fontWeight="bold" className="callsHeaderTitle">
            ...
            <Box>
              <ul style={{ display: 'grid', gridTemplateColumns: '47.33% 33.33% 33.33%', fontWeight: 'bold', height: '40px' }}>
              ...
              {reports.map((report, index) => (
                ...
                style={{
                  display: 'grid',
                  gridTemplateColumns: '34.33% 43.33% 33.33%',
                  cursor: 'pointer',
...
export default DashboardReportsMissedCalls;
