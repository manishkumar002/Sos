...
const DashboardDeactivatedElevators = () => {
  ...
  const fetchElevators = async () => {
    try {
      ...
      // Filter only Deactivated elevators
      const deactivatedElevators = data.filter(
        (elevator) => elevator.status === 'Deactivated',
      );

      setElevators(deactivatedElevators);

      if (deactivatedElevators.length > 0) {
        setSelectedElevator(deactivatedElevators[0]);
        setSelectedLiftId(deactivatedElevators[0].id);
      }
    } catch (error) {
...
export default DashboardDeactivatedElevators;
