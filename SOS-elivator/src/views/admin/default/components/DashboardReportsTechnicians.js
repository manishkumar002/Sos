...
const DashboardReportsTechnicians = () => {
  ...
  const fetchTechnicians = async (startDate, endDate) => {
    try {
  ...
  const handleDateChange = (startDate, endDate) => {
    if (startDate && endDate) {
      fetchTechnicians(startDate, endDate);
    } else {
      fetchTechnicians();
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${config.SOS_elivator}/getMonthlyCountTechnician`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
          ...
          >
            ...
            <Box>
              <ul
                style={{
                  display: 'grid',
                  gridTemplateColumns: '20.33% 45.33% 40.33%',
                  fontWeight: 'bold',
                  height: '40px',
                }}
              >
              ...
              {reports.map((report, index) => (
                <ul
                  ...
                  className={
                    report.id === selectedLiftId
                      ? 'lift-row selected'
                      : 'lift-row'
                  } // Add active class if selected
                >
                  <li>{report.techunique_id}</li>
                  <li>{report.name}</li>
...
export default DashboardReportsTechnicians;
