...
const DashboardConnectedElevators = () => {
  ...
  const fetchElevators = async () => {
    try {
      ...
      // Filter only Active elevators
      const connectedElevators = data.filter(
        (elevator) => elevator.status === 'Active',
      );

      setElevators(connectedElevators);

      // Select first elevator by default
      if (connectedElevators.length > 0) {
        setSelectedElevator(connectedElevators[0]);
        setSelectedLiftId(connectedElevators[0].id);
      }
    } catch (error) {
...
export default DashboardConnectedElevators;
