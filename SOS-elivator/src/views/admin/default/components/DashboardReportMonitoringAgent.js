...
const DashboardReportMonitoringAgent = () => {
...
useEffect(() => {
 ...
 const fetchAgents = async (startDate, endDate) => {
    try {
      const response = await fetch(`${config.SOS_elivator}/allAgent`, {
        ...
        body: JSON.stringify({agentId,startDate,endDate})
      });
      ...
      if (report.length > 0) {
        ...
        setSelectedLiftId(report[0].id); // Select the first agent by default
      }
    } catch (error) {
...
const handleSearchClick = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${config.SOS_elivator}/getMonthlyCountAgent`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          ...
          Monitoring Agents         
            ...
            <Box>
              ...
              {reports.map((report, index) => (
                ...
                style={{
                  display: 'grid',
                  gridTemplateColumns: '20.33% 30.33% 53.33%',
                  cursor: 'pointer',
...
export default DashboardReportMonitoringAgent;
