...
const DashboardReportTaskAssigned = ({ setReportsCounts }) => {
 ...
 const fetchTaskAssignedList = async (startDate, endDate) => {
  try {
    const response = await fetch(`${config.SOS_elivator}/taskAssignedList`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ elevatorId,startDate,endDate }), // Send elevatorId if provided
    });

    const result = await response.json();
    const data = result.taskAssigned || [];

    setReports(data);

    // ✅ Set default selected row after fetch
    if (data.length > 0) {
      setSelectedLiftId(data[0].id);
      setSelectedReport(data[0]);
    }
  } catch (error) {
    console.error('Failed to fetch task assigned list:', error);
    setReports([]);
  }
};
...
const handleDateChange = (startDate, endDate) => {
  if (startDate && endDate) {
    fetchTaskAssignedList(startDate, endDate);
  } else {
    fetchTaskAssignedList();
  }
};
 ...
 const [year, setYear] = useState('2025');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${config.SOS_elivator}/getMonthlyCountTasAssigned`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  <Box bg="white" boxShadow="md" borderRadius="lg" p={6} mb={0} w="100%">
    ...
    <Box>
      <ul style={{ display: 'grid', gridTemplateColumns: '33.33% 43.33% 43.33%', fontWeight: 'bold', height: '40px' }}>
      ...
      {reports?.map((report, index) => (
        <ul
          ...
          className={report?.id === selectedLiftId ? 'lift-row selected' : 'lift-row'}
        >
          <li>{report?.elevator_id}</li>
          <li>{report?.technician?.name}</li>
          <li>{new Date(report.assigned_date).toLocaleDateString('en-GB')}</li>
        </ul>
      ))}
    </Box>
  </Box>
...
export default DashboardReportTaskAssigned;
